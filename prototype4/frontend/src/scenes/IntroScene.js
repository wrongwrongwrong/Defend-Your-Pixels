...
export class IntroScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (this.ws) {
      this._boardStateHandler = d => {
        if (d.phase === "tutorial") {
          this._goToScene("Tutorial", d);
        } else if (d.phase !== "side_selection") {
          this._goToScene("Game", d);
        }
      };
  ...
  _makeCard(x, y, title, tag, titleColor, bgColor, rimColor, bullets, onChoose) {
    ...
    if (onChoose) hitArea.on("pointerdown", onChoose);

    return container;  // return container so the tween can animate it
  }

  _goToScene(sceneName, initialState) {
    ...
    this.cameras.main.once("camerafadeoutcomplete", () => {
      this.scene.start(sceneName, { ws: this.ws, initialState });
    });
  }
